...
test('rendered stems are aligned and direct arrival plus early plus late sums to wet', async () => {
  const audio = await renderExportAudio({
    ...
    inputMono: new Float32Array([1, .5]), settings: { durationSeconds: .5, fdnTailSeconds: .4, lateMode: 'convolution', maxBounces: 2, earlyPathLimit: 32, cutoffDb: -90, lateWalks: 16, tailPersistence: .6, fdnDensity: .7, fdnDamping: .55, geometryInfluence: .7 }
  });
...
test('co-located direct arrival starts at sample zero without a separate trigger', async () => {
  const audio = await renderExportAudio({
    ...
    settings: { durationSeconds: .01, fdnTailSeconds: .01, lateMode: 'convolution', maxBounces: 2, earlyPathLimit: 32, cutoffDb: -90, lateWalks: 1, tailPersistence: .6, fdnDensity: .7, fdnDamping: .55, geometryInfluence: .7 }
  });
...
test('distinct direct stem contains trigger and propagated arrival while wet excludes trigger', async () => {
  const audio = await renderExportAudio({
    ...
    reflectors: [], inputMono: new Float32Array([1]),
    settings: { durationSeconds: .5, fdnTailSeconds: .5, lateMode: 'convolution', maxBounces: 2, earlyPathLimit: 32, cutoffDb: -90, lateWalks: 1, tailPersistence: .6, fdnDensity: .7, fdnDamping: .55, geometryInfluence: .7 }
  });
